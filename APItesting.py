...
try:
    #print("嘗試開啟設定檔")
    setting = open("C:/Users/385225/BX api/setting2.txt").read().split("\n")
...
def run_trade():
    ...
    balance = float(get_balance()["data"]["balance"]["balance"])
    print_info()
    set_leverage(bx_symbol , 1 , "LONG")
    set_leverage(bx_symbol , 1 , "SHORT")
        ...
        try:
            if  mode == "none":
             ...
             #       amt = float( 3.8 * leverage * x ) / bx_price
                ...
                #elif    True :
                        #print("BN : " +  str(bn_price) + " | BX :  " + str(bx_price) )
            elif mode == "long" and (bn_price <= bx_price ):
            ...
            elif  mode == "short" and (bx_price <= bn_price ) :
